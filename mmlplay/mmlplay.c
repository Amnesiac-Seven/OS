...
int strtol(char *s, char **endp, int base);	/* ?y”Ÿ”stdlib.h */
void waittimer(int timer, int time);
void end(char *s);

void HariMain(void){
	/*t:?‹È‘¬“x  l:àÒ?‰¹?(İ'L'@)  o:”ª“x  q:‰¹•„‰‰‘t•û®  note_old:ãˆê‰¹?*/
...
file_error:
	...
	i = 0;//??•„ 0:•’Êš•„ 1:’i’?•„  2:s’?•„  3:š•„‹ø,?—"12ashd"
		...
		if(i == 0 && *p > ' '){
			if(*p == '/'){
				if(p[1] == '*'){
					...
					*r = *p;
					if('a' <= *p && *p <= 'z'){//‘S•”‰ü¬‘åÊš•„
						*r += 'A' - 'a';
					}
					r++;
				}
			}else if(*p == 0x22){// 0x22??š•„"
				*r = *p;
				r++;
				i = 3;
			}else {
	...
	for(;;){
		if(('A' <= *p && *p <= 'G') || *p == 'R'){/*‰¹•„A‹x~•„*/
			...
			if(i != note_old){//?‰¹?—^ãˆê‰¹?•s“¯
				...
				if(28 <= note_old && note_old <= 107)//•¢á³ãˆê‰¹?
			...
			if(p[1] == 'K'){
				...
				s[i] = 0;
				if(i != 0)
					api_putstrwin(win, 128 - i * 4, 88, 0, i, s);//—v?¦“Iš•„?s‹’†“W¦
